...
u16 GetEvolutionTargetSpecies(struct Pokemon *mon, u8 mode, u16 evolutionItem, struct Pokemon *tradePartner)
{
    ...
    case EVO_MODE_NORMAL:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_FRIENDSHIP_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= DAY_START && gLocalTime.hours < NIGHT_START && friendship >= 220) //edited for new night/day cycle
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_DAY:
                RtcCalcLocalTime();
    ...
    case EVO_MODE_ITEM_CHECK:
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_ITEM_NIGHT:
                RtcCalcLocalTime();
                if ((gLocalTime.hours >= NIGHT_START || gLocalTime.hours < DAY_START) < 12 && gEvolutionTable[species][i].param == evolutionItem)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;   
            }
        }
            ...
            if (method == formChanges[i].method && species != formChanges[i].targetSpecies)
            {
                ...
                case FORM_ITEM_USE:
                    if (arg == formChanges[i].param1)
                    {
                        ...
                        case NIGHT:
                            RtcCalcLocalTime();
                            if ((gLocalTime.hours >= NIGHT_START || gLocalTime.hours < DAY_START) < 12)
                                targetSpecies = formChanges[i].targetSpecies;
                            break;
                        default:
                            targetSpecies = formChanges[i].targetSpecies;
